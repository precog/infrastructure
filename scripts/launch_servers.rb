...
def build_server(hostname, group, zone, instance_type, sda_size, data_size, roles, params = {}) 
  ...
  filename = "user-data/#{hostname}-init.sh"
  File.open(filename, 'w') do |f| 
    f.puts(
      build_appserver_user_data("user_data.sh.erb", 
        {
          :hostname => hostname,
  ...
  while (hosts < quantity) 
    ...
    conf = <<-ENDSCRIPT
      config = { 
        "_id": "#{set_id}",
        members: [
          {"_id": 0, host: '#{hostbase}#{"%02d" % (host_id)}.reportgrid.com:27018'},
          {"_id": 1, host: '#{hostbase}#{"%02d" % (host_id + 1)}.reportgrid.com:27018'},
          {"_id": 2, host: '#{hostbase}#{"%02d" % (host_id + 2)}.reportgrid.com:27018', arbiterOnly:true}
        ] 
      };
      rs.initiate(config);
    ENDSCRIPT
...
build_appserver_set("appserver", 4, 3)
#build_mongo_replicasets("mongodb", 7, 3)
