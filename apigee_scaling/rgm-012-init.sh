...
#echo 'finished resize' >> /tmp/reportgrid_setup.log


#echo 'starting mongo setup' >> /tmp/reportgrid_setup.log
...
#echo 'finished mongo setup' >> /tmp/reportgrid_setup.log
...
#echo 'chef installed' >> /tmp/reportgrid_setup.log
...
#echo 'starting initial chef run' >> /tmp/reportgrid_setup.log

# write temporary json file
/bin/cat <<END > ${JSON_ATTRIBUTES_FILE}
{"run_list": [ "role[default]", "role[mongodb-shard-server]", "role[mongodb-replset-server]" ] }
END

# do initial (non-daemonized) chef run
/usr/bin/chef-client -L /var/log/chef/client.log -l debug -j ${JSON_ATTRIBUTES_FILE}

#echo 'initial chef run complete' >> /tmp/reportgrid_setup.log
...
/bin/cat <<END > ${MONGO_INIT_FILE}
      config = { 
        _id: 4,
        members: [
          {_id: 0, host: 'rgm-012.reportgrid.com:27018'},
          {_id: 1, host: 'rgm-013.reportgrid.com:27018'},
          {_id: 2, host: 'rgm-014.reportgrid.com:27018, arbiterOnly:true"}
        ] 
      };
      rs.initiate(config);
